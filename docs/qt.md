...
#### Several ABIs in one package

One APK for every device needs one environment per ABI, and one settings
file describing them all. `deployment_settings` and
`android_deployment_settings` take a mapping of ABI to environment in place
of an environment, plus the ABI that is primary:

```python
from pcons.toolchains.qt.android import android_output_dir

ABIS = {"arm64-v8a": ARM64_QT, "x86_64": X86_64_QT}
PRIMARY = "arm64-v8a"

envs, apps = {}, {}
for abi, qt_root in ABIS.items():
    env = project.Environment(toolchain="llvm", name=f"android-{abi}")
    env.build_prefix = f"android-{abi}"
    env.apply_cross_preset(android(ndk=NDK, arch=abi, api=28, sdk=SDK))
    qt = find_qt(project, env, modules=["Quick"], qt_root=qt_root,
                 probe="qtpaths")
    envs[abi] = env
    apps[abi] = project.QtSharedLibrary("myapp", env, sources=["main.cpp"],
                                        link=[qt.Quick])

settings = android_deployment_settings(
    project, envs, app="myapp", primary=PRIMARY, ...
)

package = android_output_dir(envs[PRIMARY], "myapp")
staged = [
    stage_application_library(project, envs[abi], app=app, output=package)
    for abi, app in apps.items()
]
apk = android_apk(project, envs[PRIMARY], app="myapp", settings=settings,
                  staged=staged)
```

Two things the environments need, and neither is optional:

- **a name**, which is what lets both hold a target called `myapp`. Without
  it pcons refuses the second as a duplicate.
- **a `build_prefix`**, which is what keeps their outputs apart. Without it
  both link `build/libmyapp.so` and pcons refuses two producers of one file.

**Every ABI stages into the same output directory**, the primary's, because
androiddeployqt owns one directory and reads one `libs/<abi>/` out of it per
ABI. `android_apk` then takes the list as `staged=` and waits for all of
them; a missing one is a package built successfully with half the
architectures in it.

Seven keys become maps of ABI to value: `qt`, `architectures` and the five
`qt*Directory` ones. Nothing else does. In particular `abi`,
`extraPrefixDirs` and the host tool paths name the **primary** ABI alone,
and `application-binary` stays one plain name -- androiddeployqt appends the
ABI suffix itself. All of that is measured against a multi-ABI settings file
Qt's own CMake wrote at 6.11.1, not inferred from `qt` becoming a map.

`primary` is asked for rather than read off the first key, so reordering the
mapping cannot silently repoint those three. It may be left out only when
the mapping has one entry.

The keys androiddeployqt reads once for the whole package are the union over
every environment: `extraLibraryDirs` names each ABI's library directories,
and `qml-root-path` and `qml-import-paths` cover every ABI's QML. The scan
is skipped only when no environment has a `QtQmlModule` at all.

The presets must agree on the NDK and the SDK, since the file states one of
each; they are compared and a disagreement raises.
