...
2. Freestanding compile and link flags, mixed C and C++, no libc, no libstdc++.
3. A static library (a board-support layer) archived with arm-none-eabi-ar.
4. One library built for both worlds with target.build_for(): the checksum the
   firmware speaks on the wire is compiled freestanding for the MCU and again
   with the host compiler, so a host program can test the algorithm before
   anything is flashed.
5. A host-built generator producing a header the firmware includes.
6. A linker script passed inside a -T flag, wrapped in PathToken so the
   generator relativizes it, plus an implicit dep so editing it relinks.
7. Post-link artifacts: .bin, .hex and a size report, as env.Command steps.
8. A user command, `pcons run qemu`, that boots the image.
...
# the arm-none-eabi binaries and adds the freestanding Cortex-M3 flags.
...
env.cc.includes.append(project.build_dir / "gen")


# The host environment builds what runs on the build machine. Its name is the
# subdirectory a build_for() copy writes to, further down.
host_env = project.Environment(toolchain="gcc", name="host")
genver = project.Program("genver", host_env, sources=["tools/genver.c"])

# cwd= makes $SOURCE expand to "build/genver" rather than a bare "genver",
# which the shell would look up on PATH instead of executing.
...
# can test the algorithm before anything is flashed. build_for() copies the
# target as it stands, so the include dir above is shared by both builds and
# anything written below belongs to one of them.
#
# The copy lands in build/host/, from the host environment's name. The cross
# targets are not copies, so the firmware stays at build/firmware.elf.
common = project.StaticLibrary("common", env, sources=["src/common.c"])
common.public.include_dirs.append(project.root_dir / "src")
common_host = common.build_for(host_env)
...
@cli_command()
def qemu() -> None:
    """Run the firmware in QEMU (build it first with `pcons`)."""
    import shutil
