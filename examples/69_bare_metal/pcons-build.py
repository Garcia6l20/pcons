...
4. One library built for both worlds from a function of the environment: the
   ...
   algorithm before anything is flashed.
...
8. A user command, `pcons run qemu`, that names the firmware as a dependency
...
# subdirectory common_lib() writes to, further down.
...
#
# A plain function of the environment is all it takes. Target names must be
# unique, so the environment's name goes into the target name and output_name
# puts the file back to common. output_prefix replaces the platform's "lib",
# so it carries the directory and the "lib" both, keeping the two builds apart:
# build/target/libcommon.a and build/host/libcommon.a. Nothing here is a pcons
# feature, so the same function can vary sources or flags per environment.
def common_lib(environment):
    lib = project.StaticLibrary(
        f"common-{environment.name}", environment, sources=["src/common.c"]
    )
    lib.output_name = "common"
    lib.output_prefix = f"{environment.name}/lib"
